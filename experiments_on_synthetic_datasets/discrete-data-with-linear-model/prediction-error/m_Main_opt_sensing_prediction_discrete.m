...
k_hub = 20;    % pre-given number of hubs (sensor locations)
...
[network, Hub_list] = f_netGen_given_k_discrete(nodeNum, k_hub, sparsity); % scaled network and sorted hub list

%% step2:generate epidemic dynamics %%
trai_sampleTimeLength = 65;  % sample number in training dateset
test_sampleTimeLength = 65;   % sample number in testing dateset
BER = 0.05; % noise level
periodInterval = 5;
[trai_Phi,trai_Y,trai_x,trai_y,Wgen] = f_dynamicsGen_compact_sparse_discrete(trai_sampleTimeLength, network,periodInterval,BER);
[test_Phi,test_Y,test_x,test_y,~] = f_dynamicsGen_compact_sparse_discrete(test_sampleTimeLength, network,periodInterval,BER);
...
for i = 1:len_k
    ...
    % %=== SNMA ===%
    tic
    [ SNMA_est_y_groupxi, SNMA_sel_set_groupxi,est_network] = f_SNMA_method_discrete_groupxi(trai_x,trai_y,budget_k,test_x);
    [ precision_SNMA_groupxi, failure_rate_SNMA_groupxi] = f_evaluatation_discrete(SNMA_est_y_groupxi,test_y,Hub_list,SNMA_sel_set_groupxi);
    rmse_tmp = [rmse_tmp,precision_SNMA_groupxi];
    toc

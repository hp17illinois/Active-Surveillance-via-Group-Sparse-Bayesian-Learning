...
budget_k =10; % sentinel/sensor number according to budget.
...
[network, Hub_list] = f_netGen_given_k_basic_fun_embd(nodeNum, k_hub, sparsity,NumBasicFunc); % scaled network and sorted hub list
...
for i_fold = 1:k_fold
    ...
    test_sampleTimeLength = 30;   % sample number in testing dateset
    
    [trai_x,trai_x_embd, trai_y , Wgen] = f_dynamicsGen_compact_poly_fun_embd(trai_sampleTimeLength,network,NumBasicFunc); % a simple basic function x^2
    [test_x,test_x_embd, test_y , ~] = f_dynamicsGen_compact_poly_fun_embd(test_sampleTimeLength,network,NumBasicFunc);
    ...
    %=== SNMA ===%
    tic
    [ SNMA_est_y, SNMA_sel_set,est_network] = f_SNMA_method_embd( trai_x_embd,trai_y,budget_k,test_x_embd,NumBasicFunc);

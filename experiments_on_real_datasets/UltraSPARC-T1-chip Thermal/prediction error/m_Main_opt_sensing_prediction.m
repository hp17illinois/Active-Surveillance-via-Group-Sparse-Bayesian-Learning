...
budget_k_list = [5,10,15,20,25,30,35,40,45]; % sentinel/sensor number according to budget.
[~, len_k] = size(budget_k_list);

%% step2:load real epidemic dynamics %%
...
for i = 1:len_k
    ...
    tic
    NumBasicFunc =1;
    [ SNMA_embd_est_y, SNMA_embd_sel_set,~,gamma_value_embd] = f_SNMA_method_embd( trai_x_embd,trai_y,budget_k,test_x_embd,NumBasicFunc);
    ...
    %[ est_y, Gl_sel_set,w_inf_gl ] = f_group_lasso_method(trai_Phi,trai_Y,budget_k,test_x,nodeNum);

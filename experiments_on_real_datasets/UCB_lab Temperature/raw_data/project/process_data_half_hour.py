...
for line in f.readlines():
    try:
        ...
        continue
    md = date.split('-',1)[1]
    hour,mins,senconds = time.split(':')
    mins = int(mins)
    if mins > 30:
        min_tag = '0'
    else:
        min_tag = '1'
    date_hour_id = md + ' ' + hour + ' ' + min_tag
...
for s_id in sensor_ave_t_dict:
    for date_hour_id in sensor_ave_t_dict[s_id]:
        if date_hour_id > '03-05 16 0' and date_hour_id < '03-10 00 1':
...
'''
#test are there all the sensors in the list.
for date_hour_id in sensor_data_time_dict:
    if len(sensor_data_time_dict[date_hour_id]) != 51:
        print(len(sensor_data_time_dict[date_hour_id]))
'''


with open('Data_lab_half_hour.csv', 'w', newline='') as csvfile:
